        ...
        while (true) {
            ...
            if (Pattern.matches("\\d+ \\d+", moveInput)) {
                ...
                if (from >= 0 && from < 24 && (weighed_to > 23 || weighed_to < 0) ) {
                    return new Move[] { new Move(from, weighed_to, Move.MoveType.BEARINGOFF, brick) }; // To is either 24 or -1, so we can calculate the roll (from -> to) properly from both side
                }
                ...
                int to = Integer.parseInt(positions[1]);
                if (to >= 0 && to < 24) {
                    return new Move[] { new Move(brick == Brick.WHITE ? -1 : 24, to, Move.MoveType.REENTRY, brick) }; // From is either -1 or 24, so we can calculate the roll (from -> to) properly from both side
                }
            } else {
