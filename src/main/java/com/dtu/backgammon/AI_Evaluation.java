...
public class AI_Evaluation {
    ...
    private static final int NUM_GAMES = 10; // Number of games to simulate
    private static final Random RANDOM = new Random();
    public static FileWriter moveLogWriter;  // Declare writer here so it can be used globally
    public static FileWriter resultLogWriter;
    public static void main(String[] args) throws IOException {
        // Initialize FileWriter here before starting the games
        moveLogWriter = new FileWriter("log.txt");
        ...
        for (int i = 1; i <= NUM_GAMES; i++) {
            ...
            if (winner instanceof AI_ExpectiMax) {
                expectimaxWins++;
                resultLogWriter.write(i + ",Expectimax," + turns + "," + expectimaxWins + "," + monteCarloWins + "\n");
            } else if (winner instanceof AI_MonteCarlo) {
                monteCarloWins++;
        ...
        resultLogWriter.close();  // Make sure to close the writer after the loop finishes
        moveLogWriter.close();
    ...
    private static Player[] setupAIPlayers(Board board) {
        ...
        if (RANDOM.nextBoolean()) {
            ai1 = new AI_ExpectiMax(Brick.WHITE);
            ai2 = new AI_MonteCarlo(Brick.BLACK);
        } else {
            ai1 = new AI_MonteCarlo(Brick.WHITE);
            ai2 = new AI_ExpectiMax(Brick.BLACK);
        }
